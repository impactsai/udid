...
import { encode, type Capability } from '@ucans/core';
...
export type DemoData = { root: JWK; agent: JWK; audience: JWK; evaluator: JWK; issuer: string; payerKey: Hex; apiKey: string; config: unknown };

export async function loadDemo(): Promise<DemoData> {
  return JSON.parse(await readFile(".demo.json", "utf8")) as DemoData;
}

export async function createDemoPayment(client: FacilitatorClient, demo: DemoData) {
  const config = configSchema.parse(demo.config), tenant = config.tenants[0]!, rule = tenant.rules[0]!;
  const offer = await client.createOffer({ resource: { url: 'https://resource.example.test/weather', description: 'Demo weather report', mimeType: 'application/json' },
  ...
  const authority = offer.paymentRequired.extensions[UCAN] as { audience: string; capability: Capability };
  ...
  const payment: Payment = { x402Version: 2, resource: offer.paymentRequired.resource, accepted: requirements,
